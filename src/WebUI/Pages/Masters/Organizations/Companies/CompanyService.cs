...
{
    public class CompanyService : DataAdaptor
    {
        private readonly HttpClient _http;
        private readonly Microsoft.AspNetCore.Components.NavigationManager _navigationManager;
        private const string _api = "/api/Company";
        private const string _currentUrl = "/Master/Organizations/Companies";
        public IList<CompanyDto> DataSource { get; set; } = new List<CompanyDto>();

        #region [ Constructor ]

        public CompanyService(HttpClient httpClient, Microsoft.AspNetCore.Components.NavigationManager navigationManager) : base()
        {
            _http = httpClient;
        ...
        public override async Task<object> ReadAsync(DataManagerRequest dataManagerRequest, string key = null)
        {
            ...
            var json = await _http.GetFromJsonAsync<CompanyVm>(_api);
            DataSource = json.Companies;
            IEnumerable<CompanyDto> _GridData = DataSource;
            int _Count = 0;
            ...
            _Count = _GridData.Cast<CompanyDto>().Count();
        ...
        {
            var dto = value as CompanyDto;
            var response = await _http.PostAsJsonAsync<CompanyDto>(_api, dto);
            return value;
        }
        ...
        public override async Task<object> UpdateAsync(DataManager dataManager, object value, string keyField, string key)
        {
            ...
            var dto = (value as CompanyDto);
            var response = await _http.PutAsJsonAsync<CompanyDto>($"{_api}/{dto.CompanyId}", dto);
